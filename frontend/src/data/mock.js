...
export const projects = [
  {
    ...
    featured: true,
    achievements: [
      "Live production e-commerce platform running real-money customer transactions since October 2025. One-principal art business based in Dubai, run from Mumbai. Scale: ~12 Postgres tables, ~30 forward-only migrations, ~22 API routes, ~26 pages, ~50 React components — all built end-to-end by one person.",
      "Distinctive payment architecture — dual-rail checkout: USDT-on-Polygon (verified on-chain via ethers + a Vercel Cron scanner walking USDT Transfer events on the receiving wallet) and bank transfer (admin manual confirm). Race-locked atomic confirmation; block-timestamp deadline authority with a shared 60s grace across verify / cron / status routes. (Engineering detail — micro-offset payment discriminator, SELECT FOR UPDATE row-lock, cascading status-triggers — lives in the case study.)",
      "AI-native engineering workflow — Claude Code as a sparring partner for threat modeling, adversarial review, and code generation. The system was built and audited through this workflow: three documented audit rounds, ~11 security commits, five new migrations (027–031) covering DB-backed rate limiting, admin brute-force lockout, CSRF Origin checks, PII redaction, and HMAC-secret isolation. AI handles the typing; design choices, threat model, and rollout sequencing are mine.",
      "Server-side growth infrastructure — Meta Pixel + Conversions API across every conversion surface (checkout, inquiry / contact / project-request forms, newsletter footer, product pages, site-wide PageView), all dedup-aware via shared eventID, PII hashed SHA-256 per Meta spec. Plus a transactional email pipeline (Resend, six lifecycle kinds, append-only audit table, admin resend UI synced with the API route). The kind of stack typically owned by a dedicated growth-engineer hire.",
      "Operations (the non-code half) — run Meta ad campaigns end-to-end, manage limited-edition print production with printing houses, coordinate fulfillment from the Dubai warehouse, hire and direct video editors, sound design + multi-platform posting (Behance / Tumblr / Instagram), use Claude Code as a creative production tool to scale Reels generation."
    ]
  },
  {
    id: 2,
